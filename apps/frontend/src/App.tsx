...
export default function App() {
  ...
  async function refreshCameras() {
    ...
    try {
      await refresh();
      // Remount every MJPEG image so stalled browser connections retry, even if camera metadata is unchanged.
  ...
  return <div className="app-shell">
    ...
    <main className="dashboard">
      ...
      <InsightsPanel />
      <section className="section-header"><div><p className="eyebrow">CÁMARAS</p><h2>Vista en directo</h2></div><div className="top-actions"><span className="hint">Vista JPEG · {cameras.length} fuentes configuradas</span><button type="button" className="ghost" disabled={refreshing} onClick={() => { void refreshCameras(); }} title="Actualizar estados y reconectar los vídeos en directo">{refreshing ? '↻ Recargando…' : '↻ Recargar cámaras'}</button></div></section>
      ...
      {visible.length ? <div className={'camera-grid ' + (visible.length === 1 ? 'single' : '')}>{visible.map(camera => <CameraCard key={camera.id} camera={camera} large={selected !== 'all'} onSelect={setSelected} streamRevision={streamRevision}/>)}</div> : <div className="empty">No hay cámaras para esta vista.</div>}
